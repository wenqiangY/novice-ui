...
Novice UI 通过使用 `n-config-provider` 调整主题。
...
将 `n-config-provider` 的 `theme` 设为从 novice-ui 导入的 `darkTheme` 来设定暗色主题。
...
通过设定 `n-config-provider` 的 `theme-overrides` 来调整主题变量。novice-ui 导出了 `GlobalThemeOverrides` 类型帮助你定义主题。

具体可使用变量请参考 `GlobalThemeOverrides` 类型提示。

如果想要查看更多的主题变量，可在 Novice UI 主页的右下角的 edit 按钮查看。
...
<script>
  ...
  /**
   * js 文件下使用这个做类型提示
   * @type import('novice-ui').GlobalThemeOverrides
   */
...
```html
<script lang="ts">
  import { NConfigProvider, GlobalThemeOverrides } from 'novice-ui'
...
```html
<script lang="ts">
  import { SelectProps, ButtonProps } from 'novice-ui'
...
```html
<script>
  import { NConfigProvider, darkTheme } from 'novice-ui'
...
1. novice-ui 会设定一些非响应式的全局样式（例如字体），它们在默认状况下工作良好，但是不能响应主题的变化。
...
novice-ui 提供主题编辑器帮助你方便的编辑主题并导出对应配置。它可以被嵌套于 `n-config-provider` 中。

主题编辑器不包含在全局安装中（`app.use(novice)`）。你需要显式引入来使用它。
...
<script>
  import { defineComponent } from 'vue'
  import { NThemeEditor } from 'novice-ui'
